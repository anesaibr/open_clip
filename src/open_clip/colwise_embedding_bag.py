...
class xFormerEmbeddingBag(nn.Module):
    ...
    def forward(self, indices, scores):
        if torch.isnan(indices).any() or torch.isinf(indices).any():
            print("[DEBUG:xFormer] NaNs/Infs in indices")
        if torch.isnan(scores).any() or torch.isinf(scores).any():
            print("[DEBUG:xFormer] NaNs/Infs in scores")
        # print(f"[DEBUG:xFormer] indices shape: {indices.shape}, scores shape: {scores.shape}")
        ...
        if torch.isnan(output).any() or torch.isinf(output).any():
            print("[DEBUG:xFormer] output has NaNs/Infs")
