...
# from lingua.float8 import convert_linears_to_fp8 #FIXME: Uncomment this line if 'distributed_args.float8_recipe' is used

logger = logging.getLogger()

# for selective AC
# default_no_recompute_ops = { #FIXME: torch.ops.xformers_flash.flash_fwd.defaul unrecognized
...
class DistributedArgs:
    ...
    tp_size: int = 1 # Tensor Parallel (TP) used if you want to do model parallel across GPUs in addition to data parallel
    memory_parallel_size: int = 1 # If you want to horizontally shard your memory layers across multiple GPUs 
    selective_activation_checkpointing: bool = False # wraps your model with a selective checkpoint approach.

    checkpoint_each_layer: bool = False # wraps each layer with a standard PyTorch checkpoint.
    compile: bool = False
    fsdp_type: str = "no_shard" # full_shard or no_shard, full_shard gives largest memory savings but can be slower due to extra communication.
    model_dtype: str = "bf16" # The param dtype you want to store your model weights in.
    float8_recipe: Optional[str] = None # an advanced feature for further memory/compute savings.
...
def parallelize_model(
    ...
    for name, submodule in model.named_modules(): # Instead of referencing `model.layers`, doing a submodule scan
        if hasattr(submodule, "mp_parallelize"):
            ...
            submodule.mp_parallelize(memory_mesh, model_args, distributed_args, param_dtype)
    ...
    if (
        ...
        if fsdp_grouping_plan is None:
            ...
            fsdp_grouping_plan = []  # no layer-level grouping
        ...
        # Log the mesh and config
        rank = torch.distributed.get_rank() if torch.distributed.is_initialized() else 0
        logging.info(f"Rank {rank}: fsdp_config details:")
        logging.info(f"  mp_policy: {fsdp_config['mp_policy']}")
        try:
            mesh = fsdp_config["mesh"]
            # Attempt to log mesh shape and size if available
            mesh_shape = mesh.mesh.shape if hasattr(mesh, "mesh") else "N/A"
            mesh_size = mesh.size() if hasattr(mesh, "size") else "N/A"
            logging.info(f"  mesh: {mesh}, shape: {mesh_shape}, size: {mesh_size}")
        except Exception as e:
            logging.info(f"  mesh info unavailable: {e}")
        
        model = fully_shard(model, **fsdp_config, reshard_after_forward=True) #fully_shard the entire model as a single group
