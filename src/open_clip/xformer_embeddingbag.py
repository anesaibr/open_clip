...
# responsible for efficiently computing gradients for the memory/embedding bag during the backward pass
def embedding_bag_bw_rev_indices( 
    ...
    weight_grad = torch.empty_like(weight)
    per_sample_weights_grad = torch.empty_like(per_sample_weights)
    BLOCK_SIZE = 8 # a common tuning parameter for GPU kernels that partitions the work efficiently on the GPU
