...
from open_clip_train.train import train_one_epoch_long_short, evaluate_long_short_caps
# from open_clip import TextTransformerRoPE, precompute_freqs_cis_dynamic_ntk_scaling, CLIP, text_global_pool, _expand_token
from open_clip.memory import ProductKeyArgs,HashingMemory
# from open_clip.models import CLIP
# from open_clip.transformers import VisionTransformer
from src.open_clip.transformer import VisionTransformer
...
def main(args):
    ...
    if args.name is None:
        ...
        args.name = '-'.join([
            date_str,
            f"finetune_memory_{model_name_safe}"
            f"lr_{args.lr}",
    ...
    # #  === DEBUG: Inspect model keys and architecture ===
    # # 1) List all top‐level attributes on model.visual
    # print("model.visual children:", [name for name, _ in model.visual.named_children()])

    # # 2) List all modules under model.visual
    # print("model.visual modules:", [name for name, _ in model.visual.named_modules()])

    # # 3) List all parameters under model.visual
    # print("model.visual parameters:", [name for name, _ in model.visual.named_parameters()])

    # # 4) List the full state_dict keys (flattened):
    # print("Full state_dict keys:", list(model.state_dict().keys()))
    ...
    ckpt = torch.load(args.student_model, map_location="cpu")
    ...
    if args.distributed and not args.horovod:
        ...
        # Broadcast each parameter and buffer from Rank 0 to all other ranks
        state_dict_to_sync = model.state_dict()
        for param_name, tensor_to_sync in state_dict_to_sync.items():
            tensor_to_sync = tensor_to_sync.to(device)
    ...
    if args.distributed and not args.horovod:
        if args.use_bn_sync:
            logging.info(f"Rank {args.rank}: Applying SyncBatchNorm...") # Added log for clarity
        ...
        logging.info(f"Rank {args.rank}: DDP wrapping complete.") # Added log for clarity
    #--- DDP Synchronization End ---
    ...
    if 'train' not in data:
        ...
        # Evaluate.
        evaluate_long_short_caps(model, data, start_epoch, args, tb_writer=writer, tokenizer=tokenizer)
        return
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_one_epoch_long_short(model, data, loss, epoch, optimizer, scaler, scheduler, dist_model, tokenizer, args, tb_writer=writer)
        completed_epoch = epoch + 1

        if any(v in data for v in ('val', 'imagenet-val', 'imagenet-v2')):
            evaluate_long_short_caps(model, data, completed_epoch, args, tb_writer=writer, tokenizer=tokenizer)
        ...
        if args.save_logs:
            ...
            if completed_epoch == args.epochs or (
                ...
                # wandb.save(os.path.join(args.checkpoint_path, f"epoch_{completed_epoch}.pt"))
