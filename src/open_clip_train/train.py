...
def first_sentence_str(s: str):
    """Cleans and extracts the first sentence from a decoded string."""
    # Remove special tokens that might persist after decoding
    s = s.replace('<|startoftext|>', '').replace('<|endoftext|>', '').strip()
    # Split by sentence-ending punctuation and return the first part
    parts = re.split(r'(?<=[\.\?\!])\s+', s, maxsplit=1)
    return parts[0]

# def tokens_to_first_sentence(tokens, tokenizer,max_length):
#     # 1) decode to text
#     txt = tokenizer.decode(tokens.tolist())         # List[int] -> str
#     # 2) extract first sentence (a.k.a short caption)
#     first = first_sentence_str(txt)
#     # 3) re-tokenize to get only input_ids
#     ids = tokenizer.encode(first)                   # List[int]

#     # return out['input_ids'].squeeze(0)
#     return torch.tensor(ids, dtype=torch.long)

def tokens_to_first_sentence(tokens: torch.Tensor, tokenizer, max_length: int) -> torch.Tensor:
    """
    Decodes a tensor of token IDs, extracts the first sentence, re-encodes it,
    and returns a padded/truncated 1D tensor of fixed length.
    """
    # 1. Decode tensor to a string
    # Use .cpu().numpy() as tokenizer.decode expects a list or numpy array
    decoded_text = tokenizer.decode(tokens.cpu().numpy())

    # 2. Extract the first sentence (a.k.a short caption) from the cleaned string
    first_sentence = first_sentence_str(decoded_text)

    # 3. Re-tokenize using the tokenizer's __call__ method.
    # This returns a 2D tensor of shape [1, context_length]
    short_tokens_tensor_2d = tokenizer(first_sentence, context_length=max_length)

    # 4. Remove the first dimension to get a 1D tensor of shape [context_length].
    return short_tokens_tensor_2d.squeeze(0)

# This function is similar to train_one_epoch, but it handles both long and short texts. Used for Fine-tuning script.
def train_one_epoch_long_short(model, data, loss, epoch, optimizer, scaler, scheduler, dist_model, tokenizer, args, tb_writer=None):
    device = torch.device(args.device)
    ...
    if args.accum_freq > 1:
        accum_images, accum_texts, accum_short_texts, accum_features, accum_features_short = [], [], [], {}, {}
    ...
    for i, batch in enumerate(dataloader):
        ...
        images = images.to(device=device, dtype=input_dtype, non_blocking=True)
        texts = texts.to(device=device, non_blocking=True)
        short_text = short_text.to(device=device, non_blocking=True)
        ...
        if args.accum_freq == 1:
            ...
            with torch.no_grad():
                with autocast():
                    ...
                    for f in ("logit_scale", "logit_bias"):
                        model_out_short.pop(f, None)

                    for key, val in model_out_short.items():
                        if key in accum_features_short:
                            accum_features_short[key].append(val)
                        else:
                            accum_features_short[key] = [val]

                accum_images.append(images)
                accum_texts.append(texts)
                accum_short_texts.append(short_text)
            ...
            for j in range(args.accum_freq):
                ...
                with autocast():
                    ...
                    inputs_no_accum_short = {}
                    inputs_no_accum_short["logit_scale"] = logit_scale_short = model_out_short.pop("logit_scale")
                    if "logit_bias" in model_out_short:
                        inputs_no_accum_short["logit_bias"] = model_out_short.pop("logit_bias")

                    inputs_short = {}
                    for key, val in accum_features_short.items():
                        accumulated = accum_features_short[key]
                        inputs_short[key] = torch.cat(accumulated[:j] + [model_out_short[key]] + accumulated[j + 1:])

                    losses_short = loss(**inputs_short, **inputs_no_accum_short, output_dict=True)
                    del inputs_short
                    del inputs_no_accum_short
                    
                    total_loss_long = sum(losses.values())
                    total_loss_short = sum(losses_short.values())
                    total_loss = total_loss_long * 0.8 + total_loss_short * 0.2
                    losses["contrastive_loss_short"] = total_loss_short
                    losses["loss"] = total_loss
...
#NOTE: this function is not similar to evaluate, but it handles both long and short texts. Used for Fine-tuning script.
# does not differniate between coco and sharegpt4v datasets, assumes & handles a 1-to-1 mapping only!
def evaluate_long_short_caps(model, data, epoch, args, tb_writer=None, tokenizer=None):
    metrics = {}
    if not is_master(args):
        return metrics
    device = torch.device(args.device)
    ...
    if 'val' in data and (args.val_frequency and ((epoch % args.val_frequency) == 0 or epoch == args.epochs)):
        ...
        cumulative_loss = 0.0
        cumulative_gen_loss = 0.0
        all_image_features, all_text_features, all_short_text_features = [], [], []
        with torch.no_grad():
            for i, batch in enumerate(dataloader):
                # images, texts, short_text = batch
                images, texts = batch
                ...
                with autocast():
                    ...
                    # For handling short texts
                    model_out_short = model(images, short_text)
                    image_features = model_out_short["image_features"]
                    short_text_features = model_out_short["text_features"]
                    all_short_text_features.append(short_text_features.cpu())
                    logits_per_image_short = logit_scale * image_features @ short_text_features.t()
                    logits_per_text_short = logits_per_image_short.t()
                    ...
                    total_loss = (
                        ...
                        F.cross_entropy(logits_per_text_short, labels) + 
                        F.cross_entropy(logits_per_image_short, labels))/ 2
...
def get_clip_metrics(image_features, text_features, logit_scale,calculate_mean_median_rank=False):
    ...
    for name, logit in logits.items():
        ...
        if calculate_mean_median_rank: # Control with the new argument
            metrics[f"{name}_mean_rank"] = preds_np.mean() + 1
            metrics[f"{name}_median_rank"] = np.floor(np.median(preds_np)) + 1
        for k in [1, 5, 10]:
            metrics[f"{name}_R@{k}"] = np.mean(preds_np < k)
